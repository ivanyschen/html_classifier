...
def scripts_import_v2(path_to_files):
    ...
    X_true = np.zeros((len(chars_lookup_2), 1))
    X_false = np.zeros((len(chars_lookup_2), 1))
    Y_true = np.zeros((1, 1))
    ...
    for root, dirs, files in os.walk(path_to_files):
        for name in files:
            ...
            temp_vec_x = np.zeros((len(chars_lookup_2), 1))
            ...
            for line in lines:
                ...
                for char in line:
                    ...
                    try:
                        temp_vec_x[chars_lookup_2[char.lower()], 0] += 1 if char != '\n' else 0
                    except KeyError:
                        number_of_lines -= 1
                        break
...
XT, YT, XF, YF = scripts_import_v2("data/html_vs_another/mac")

fileObject = open('XT_html_vs_another_letters.pkl','wb')
pickle.dump(XT,fileObject)
fileObject.close()

fileObject = open('XF_html_vs_another_letters.pkl','wb')
pickle.dump(XF,fileObject)
fileObject.close()

fileObject = open('YT_html_vs_another_letters.pkl','wb')
pickle.dump(YT, fileObject)
fileObject.close()

fileObject = open('YF_html_vs_another_letters.pkl','wb')
pickle.dump(YF, fileObject)
fileObject.close()

# fileObject = open('XT_html_vs_another_gist.pkl','wb')
# pickle.dump(XT,fileObject)
# fileObject.close()
#
# fileObject = open('XF_html_vs_another_gist.pkl','wb')
# pickle.dump(XF,fileObject)
# fileObject.close()
#
# fileObject = open('YT_html_vs_another_gist.pkl','wb')
# pickle.dump(YT, fileObject)
# fileObject.close()
#
# fileObject = open('YF_html_vs_another_gist.pkl','wb')
# pickle.dump(YF, fileObject)
# fileObject.close()